...
export async function readPieces() {
  const files = await globby(["catalog/cat-*.yaml/"])
  ...
  function pieceMap(pieces, piece) {
    if (piece.id in pieces) {throw new Error(`${piece.id} already exists`) }
...
export async function readBooks() {
  ...
  function bookMap(books, book) {
    if (book.id in books) { throw new Error (`${book.id} already exists`) }
    books[book.id] = book
    return books
  }
