...
export function engravePieces(pieces, format) {
  return pieces.flatMap(piece => {
    const score = stradella(render(`source/master.lys`, { pieces: [piece] }))
    return format.split(":").map(format =>
      engrave(score, `score/${piece.file}`, format)
    )
  })
}

export function engraveBooks(books, pieces, format) {
  return books.flatMap(book => {
    pieces = pieces.filter(piece =>
      book.pieces.some(bpiece => piece.file.match(bpiece))
    )
    const score = stradella(render(`source/master.lys`, { book, pieces }))
    return format.split(":").map(format =>
      engrave(score, `score/${book.file}`, format)
    )
  })
}
